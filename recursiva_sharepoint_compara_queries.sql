...
--sp_helptext sp_dba_email_aprova_notas
BEGIN    
    
SET NOCOUNT ON    
    
--tabela temporaria com a query do corpo    
create table #criarcorpo2 (BU nvarchar(100) collate Latin1_General_CI_AS_KS_WS, corpo text)    
insert into #criarcorpo2 select      
 t1.nvarchar6,     
 '<tr><td>' + t1.nvarchar1 +     
 '</td><td>' + t1.nvarchar8 +     
 '</td><td>' + t1.nvarchar5 +     
 '</td><td>' + '<a href=http://latbrspodb02/gestaodefornecedores/Financeiro/Lists/Demonstrativos/DispForm.aspx?ID=' +     
 convert(varchar, t1.tp_id) +     
 '&Source=http%3A%2F%2Flatbrspodb02%2Fgestaodefornecedores%2FFinanceiro%2FLists%2FDemonstrativos%2FAllItems%2E.aspx>Link</a>' +     
 '</td></tr>'    
...
SELECT '49', null, '3', '0', te.email, 'noreply@ipsos.com', 'Sistema de aprovação de notas', 'Aprovacao de notas (Financeiro)', 'Segue lista de Notas Fiscais cadastradas em ' + convert(varchar, getdate(), 103) + ' </ br></ br></ br> <table border="1">' + 
...
from WSS_Content_80.dbo.AllUSerData t1    
--where tp_created = getdate()    
where datediff (day, tp_Created, getdate()) = 0
and tp_isCurrent = 1    
...
--tabela temporaria com a query do corpo    
create table #criarcorpo1 (BU nvarchar(100) collate Latin1_General_CI_AS_KS_WS, corpo text)    
insert into #criarcorpo1 select      
 t1.nvarchar6,     
 '<tr><td>' + t1.nvarchar1 +     
 '</td><td>' + t1.nvarchar8 +     
 '</td><td>' + t1.nvarchar5 +     
